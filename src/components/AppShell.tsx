import { HomeIcon, MapIcon, CalendarDaysIcon, ClipboardDocumentCheckIcon, MagnifyingGlassIcon, Cog6ToothIcon } from '@heroicons/react/24/outline'
import { NavLink, Outlet } from 'react-router-dom'
import { useEffect } from 'react'
import { useAppStore } from '../store/useAppStore'

const nav = [{ to: '/', label: 'Home', icon: HomeIcon }, { to: '/timeline', label: 'Plan', icon: CalendarDaysIcon }, { to: '/map', label: 'Map', icon: MapIcon }, { to: '/packing', label: 'Pack', icon: ClipboardDocumentCheckIcon }, { to: '/search', label: 'Search', icon: MagnifyingGlassIcon }, { to: '/settings', label: 'Settings', icon: Cog6ToothIcon }]
